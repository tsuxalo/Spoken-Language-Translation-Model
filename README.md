...
- **The notebook is for exploring and explaining** what's happening at each stage, and it's built to run on Google Colab, not just locally. Not everyone on the team has a GPU on their own machine, and Colab provides free (if limited) GPU access in a browser. The first code cell selects the integration branch until the required files reach `main`, refreshes stale Colab checkouts, and installs `requirements-colab.txt`. That overlay leaves Colab's already-loaded compiled scientific stack in place; if a binary dependency nevertheless changes, the cell requests one runtime restart before model or dataset imports.
